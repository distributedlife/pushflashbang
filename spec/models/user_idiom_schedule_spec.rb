...
describe UserIdiomSchedule do
  context 'create' do
    it 'should require a user_id' do
      card_schedule = UserIdiomSchedule.new(:idiom_id => 1,
        :listening_interval => 5, :reading_interval => 5, :speaking_interval => 5, :typing_interval => 5, :writing_interval => 5,
        :listening_due => Time.now, :reading_due => Time.now, :speaking_due => Time.now, :typing_due => Time.now, :writing_due => Time.now)
    ...
    it 'should require an idiom_id' do
      card_schedule = UserIdiomSchedule.new(:user_id => 1,
        :listening_interval => 5, :reading_interval => 5, :speaking_interval => 5, :typing_interval => 5, :writing_interval => 5,
        :listening_due => Time.now, :reading_due => Time.now, :speaking_due => Time.now, :typing_due => Time.now, :writing_due => Time.now)
      card_schedule.valid?.should == false
  ...
  context 'get_next_due_for_user' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      next_schedule = UserIdiomSchedule.get_next_due_for_user(@user.id)
      next_schedule.nil?.should == true
    end

    it 'should return nil if there is no due items' do
      us1 = UserIdiomSchedule.make(:user_id => @user.id, :idiom_id => @idiom.id)
    ...
    it 'should return the most due item' do
      ...
      us2 = UserIdiomSchedule.make(:user_id => @user.id, :idiom_id => @idiom.id)
  ...
  context 'get_next_due_for_user_for_set' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set(@user.id, @set1.id)
      next_schedule.nil?.should == true
    end

    it 'should return nil if there is no due items in the set' do
      us1 = UserIdiomSchedule.make(:user_id => @user.id, :idiom_id => @idiom1.id)
      UserIdiomDueItems.make(:user_idiom_schedule_id => us1.id, :due => 5.days.from_now)
      us2 = UserIdiomSchedule.make(:user_id => @user.id, :idiom_id => @idiom2.id)
    ...
    it 'should return the most due item for the set' do
      ...
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set(@user.id, @set2.id)
  ...
  context 'get_due_count_for_user' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      count = UserIdiomSchedule.get_due_count_for_user(@user.id)
      count.should == 0
    end
  ...
  context 'get_due_count_for_user_for_set' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      count = UserIdiomSchedule.get_due_count_for_user_for_set(@user.id, @set1.id)
      count.should == 0
    end
    ...
    it 'should return the most due item' do
      ...
      count = UserIdiomSchedule.get_due_count_for_user_for_set(@user.id, @set2.id)
      count.should == 5
    end
  end
  ...
  context 'get_next_due_for_user_for_proficiencies' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      next_scheduled = UserIdiomSchedule.get_next_due_for_user_for_proficiencies(@user.id, [1])
    ...
    it 'should return nil if there is no due items in the set' do
      ...
      next_scheduled = UserIdiomSchedule.get_next_due_for_user_for_proficiencies(@user.id, [2])
      next_scheduled.nil?.should be true
      next_scheduled = UserIdiomSchedule.get_next_due_for_user_for_proficiencies(@user.id, [16])
      next_scheduled.nil?.should be true
    end
    ...
    it 'should return the most due item for the set' do
      ...
      next_scheduled = UserIdiomSchedule.get_next_due_for_user_for_proficiencies(@user.id, [2, 16])
      next_scheduled.should == us1di2

      next_scheduled = UserIdiomSchedule.get_next_due_for_user_for_proficiencies(@user.id, [16])
      next_scheduled.should == us1di5

      next_scheduled = UserIdiomSchedule.get_next_due_for_user_for_proficiencies(@user.id, [1, 2, 4, 8, 16])
  ...
  context 'get_next_due_for_user_for_set_for_proficiencies' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set_for_proficiencies(@user.id, @set1.id, [1])
      next_schedule.nil?.should == true
    end
    ...
    it 'should return nil if there is no due items in the set' do
      ...
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set_for_proficiencies(@user.id, @set1.id, [2])
      next_schedule.nil?.should == true
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set_for_proficiencies(@user.id, @set1.id, [1,2,4,8,16])
      next_schedule.nil?.should == true
    end
    ...
    it 'should return the most due item for the set' do
      ...
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set_for_proficiencies(@user.id, @set2.id, [2])
      next_schedule.should == us2di2
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set_for_proficiencies(@user.id, @set2.id, [2,8])
      next_schedule.should == us2di4
      next_schedule = UserIdiomSchedule.get_next_due_for_user_for_set_for_proficiencies(@user.id, @set2.id, [2,8,1])
      next_schedule.should == us2di4
    end
  end
  ...
  context 'get_due_count_for_user_for_proficiencies' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      count = UserIdiomSchedule.get_due_count_for_user_for_proficiencies(@user.id, [1])
      count.should == 0
    end
    ...
    it 'should return nil if there is no due items' do
      ...
      count = UserIdiomSchedule.get_due_count_for_user_for_proficiencies(@user.id, [1])
      count.should == 0
      count = UserIdiomSchedule.get_due_count_for_user_for_proficiencies(@user.id, [2])
      count.should == 0
      count = UserIdiomSchedule.get_due_count_for_user_for_proficiencies(@user.id, [16])
      count.should == 0
    end
    ...
    it 'should return the most due item' do
      ...
      count = UserIdiomSchedule.get_due_count_for_user_for_proficiencies(@user.id, [2, 4])
      count.should == 4

      count = UserIdiomSchedule.get_due_count_for_user_for_proficiencies(@user.id, [2, 4, 16])
      count.should == 6
    end
  end
  ...
  context 'get_due_count_for_user_for_set_for_proficiencies' do
    ...
    it 'should return nil if there are no scheduled items at all' do
      count = UserIdiomSchedule.get_due_count_for_user_for_set_for_proficiencies(@user.id, @set1.id, [1])
      count.should == 0
    end
    ...
    it 'should return nil if there is no due items' do
      ...
      count = UserIdiomSchedule.get_due_count_for_user_for_set_for_proficiencies(@user.id, @set1.id, [2])
      count.should == 0
      count = UserIdiomSchedule.get_due_count_for_user_for_set_for_proficiencies(@user.id, @set1.id, [1,2,4,8,16])
      count.should == 0
    end
    ...
    it 'should return the most due item' do
      ...
      count = UserIdiomSchedule.get_due_count_for_user_for_set_for_proficiencies(@user.id, @set2.id, [2])
      count.should == 1
      count = UserIdiomSchedule.get_due_count_for_user_for_set_for_proficiencies(@user.id, @set2.id, [2,4])
      count.should == 2
      count = UserIdiomSchedule.get_due_count_for_user_for_set_for_proficiencies(@user.id, @set2.id, [2,4,16])
      count.should == 3
    end
  end
end
