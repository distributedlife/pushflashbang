...
describe SetsController do
  ...
  context '"GET" due_count' do
    before(:each) do
      ...
      @user_sets = mock(UserSets).as_null_object
      UserSets.stub(:where).and_return([@user_sets])
    end

    it 'should return the due count' do
      @user_sets.should_receive(:due_count).with([1,16])  
    ...
    it 'should not include due items for different review modes' do
      @user_sets.should_receive(:due_count).with([16])  
